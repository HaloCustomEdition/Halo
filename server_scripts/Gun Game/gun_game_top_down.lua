...
-- edit --
logging = false			-- | true enable full script game logging: false = disable, true = enable, see also line 91 - can be spammy to the log, best used for tournaments/scrims/debugging
...
function ScriptLoad()
	...
	for i = 0,15 do
		if getplayer(i) then
			...
			last_damage[ip] = nil
			mybattery[i] = 1
			weapons[i] = "weapons\\plasma_cannon\\plasma_cannon"
			level[i] = 1
			playerscore[i] = 0
			team_change[i] = false
...
function Welcome(id, count, player)
	if count == 1 then
		if getplayer(player) then
			...
			privatesay(player, "This version is 'DOWN': Start with Plasma Cannon, end with Plasma Pistol.")
...
function NewGameTimer(id, count)
	...
	for x = 0,15 do
		if getplayer(x) then
			...
			beserk = false
			mybattery[x] = 0
			weapons[x] = "weapons\\plasma_cannon\\plasma_cannon"
			level[x] = 1
			playerscore[x] = 0
			...
			if m_objectId ~= nil then
				...
				if level[x] == 1 then
					battery = 0
					weapons[x] = "weapons\\plasma_cannon\\plasma_cannon"
					sendconsoletext(x, "Level 1:   Plasma Cannon", 4, 0)
					if m_object then
						writebyte(m_object + 0x31E, 0)
						writebyte(m_object + 0x31F, 0)
					end
				...
				if m_weaponId then
					...
					if m_weapon then
						...
						if logging then log_msg(1, name .. " given plasma cannon on new game.")	end
						if m_weapon then
							mybattery[x] = 1
						end
					end
		...
		if getplayer(player) then
			...
			if m_objectId ~= nil then
				...
				if level[player] == nil then
					level[player] = 1
					weapons[player] = "weapons\\plasma_cannon\\plasma_cannon"
					sendconsoletext(player, "Level 1:   Plasma Cannon", 4, 0)
					if logging then log_msg(1, name .. " given plasma cannon. Is level: " .. lvl) end
				...
				elseif level[player] == 2 then
					...
					sendconsoletext(player, "Level 2:   Sniper Rifle", 4, 0)
				...
				elseif level[player] == 3 then
					...
					sendconsoletext(player, "Level 3:   Rocket Launcher", 4, 0)
				...
				elseif level[player] == 4 then
					...
					sendconsoletext(player, "Level 4:   Pistol", 4, 0)
				...
				elseif level[player] == 5 then
					...
					sendconsoletext(player, "Level 5:   Flamethrower", 4, 0)
				...
				elseif level[player] == 6 then
					...
					sendconsoletext(player, "Level 6: Assault Rifle", 4, 0)
					if m_object then
				...
				elseif level[player] == 7 then
					...
					sendconsoletext(player, "Level 7:   Plasma Rifle", 4, 0)
					if m_object then
						writebyte(m_object + 0x31E, 0)
				...
				elseif level[player] == 8 then
					...
					sendconsoletext(player, "Level 8:   Shotgun", 4, 0)
					health = registertimer(0, "ApplyHP", player)
					if m_object then
						writebyte(m_object + 0x31E, 1)
						writebyte(m_object + 0x31F, 0)
					end
					if logging then log_msg(1, name .. " given shotgun, 1 frag. Is level: " .. lvl) end

				elseif level[player] == 9 then
					clip = needler_clip
					weapons[player] = "weapons\\needler\\mp_needler"
					sendconsoletext(player, "Level 9:   Needler", 4, 0)
					if m_object then
						writebyte(m_object + 0x31E, 0)
						writebyte(m_object + 0x31F, 2)
					end
					if logging then log_msg(1, name .. " given needler, 2 plasmas and active camouflage. Is level: " .. lvl) end

				elseif level[player] == 10 then
					weapons[player] = "weapons\\plasma pistol\\plasma pistol"
					beserk = true
					sendconsoletext(player, "Level 10:   Plasma Pistol --- FINAL LEVEL! ---", 4, 0)
					if m_object then
						writebyte(m_object + 0x31E, 1)
					...
					if logging then log_msg(1, name .. " given plasma pistol, 1 frag, 1 plasma, active camouflage removed and booster function called. Is level: " .. lvl) end
				...
				if m_weaponId then
					...
					if m_weapon then
						if (level[player] == 1) or (level[player] == 7) or (level[player] == 10) then
							mybattery[player] = 1
						else
							mybattery[player] = 0
	...
	if player then
		...
		if level[player] == 1 then
			if beserk then
				...
				setspeed(player, 0.65) -- plasma cannon 0.65
			end
		...
		elseif level[player] == 2 then
			if beserk then
				...
				setspeed(player, 0.75) -- sniper rifle 0.75
			end
		...
		elseif level[player] == 3 then
			if beserk then
				...
				setspeed(player, 0.65) -- rocket launcher 0.65
			end
			if logging then log_msg(1, name .. " speed set at 0.65. Is level: " .. lvl) end
		elseif level[player] == 4 then
			if beserk then
				...
				setspeed(player, 0.85) -- pistol 0.85
			end
		...
		elseif level[player] == 5 then
			if beserk then
				...
				setspeed(player, 1) -- flamethrower 1
			end
		...
		elseif level[player] == 6 then
			if beserk then
				...
				setspeed(player, 1.15) -- assault rifle 1.15
			end
		...
		elseif level[player] == 7 then
			if beserk then
				...
				setspeed(player, 1.15) -- plasma rifle 1.15
			end
			if logging then log_msg(1, name .. " speed set at 1.15. Is level: " .. lvl) end
		elseif level[player] == 8 then
			if beserk then
				...
				setspeed(player, 1.25) -- shotgun 1.25
			end
		...
		elseif level[player] == 9 then
			if beserk then
				...
				setspeed(player, 1.35) -- needler 1.35
			end
			if logging then log_msg(1, name .. " speed set at 1.35. Is level: " .. lvl) end
		elseif level[player] == 10 then
			setspeed(player, 1.45) -- plasma pistol 1.45
...
function OnServerChat(player, type, message)
	...
	if player then
		...
		if string.lower(message) == "help" or string.lower(message) == "@help" or string.lower(message) == "/help" then
			...
			privatesay(player, "There are 10 weapons. Starting with plasma cannon and ending with plasma pistol.")
				...
				if last_damage[vip] then
					if getplayer(killer) then
						...
						if k_objectId ~= nil then -- sets condition that killer must be alive for the following
							...
							if last_damage[vip] == "weapons\\plasma_cannon\\effects\\plasma_cannon_explosion" or last_damage[vip] == "weapons\\plasma_cannon\\effects\\plasma_cannon_melee" then
								if weapons[killer] == "weapons\\plasma_cannon\\plasma_cannon" then
									level[killer] = 2
									playerscore[killer] = 1
									if string.find(last_damage[vip], "melee") then
										say(kname .. " melee'd " .. vname .. " with a plasma cannon")
										if logging then log_msg(1, vname .. " was killed by " .. kname .. "'s melee who is level: " .. klvl) end
									else
										say(kname .. " killed " .. vname .. " with a plasma cannon")
							...
							elseif last_damage[vip] == "weapons\\needler\\detonation damage" or last_damage[vip] == "weapons\\needler\\explosion" or last_damage[vip] == "weapons\\needler\\impact damage" or last_damage[vip] == "weapons\\needler\\melee" then
								if weapons[killer] == "weapons\\needler\\mp_needler" then
									...
									beserk = true
									if string.find(last_damage[vip], "melee") then
										say(kname .. " melee'd " .. vname .. " with a needler, advanced to Level 10!")
										if logging then log_msg(1, vname .. " was killed by " .. kname .. "'s melee who is level: " .. klvl) end
									else
										say(kname .. " killed " .. vname .. " with a needler, advanced to Level 10!")
										if logging then log_msg(1, vname .. " was killed by " .. kname .. " who is level: " .. klvl) end
									end
									gameweap = registertimer(0, "AssignGameWeapons", killer)
									response = false
							...
							elseif last_damage[vip] == "weapons\\plasma pistol\\bolt" or last_damage[vip] == "weapons\\plasma rifle\\charged bolt" or last_damage[vip] == "weapons\\plasma pistol\\melee" then
								if weapons[killer] == "weapons\\plasma pistol\\plasma pistol" then
									...
									beserk = false
									if string.find(last_damage[vip], "melee") then
										say(kname .. " melee'd " .. vname .. " with a plasma pistol, advanced to WIN! +10 Points")
										if logging then log_msg(1, vname .. " was killed by " .. kname .. "'s melee who is level: " .. klvl) end
									else
										say(kname .. " killed " .. vname .. " with a plasma pistol, advanced to WIN! +10 Points")
										if logging then log_msg(1, vname .. " was killed by " .. kname .. " who is level: " .. klvl) end
									end
									response = false
								end
							...
							elseif last_damage[vip] == "weapons\\plasma grenade\\explosion" or last_damage[vip] == "weapons\\frag grenade\\explosion" then
								say(kname .. " killed " .. vname .. " with a grenade, no level advancement.")
...
Jicelle, Mandy Lynn, Misty Rain, Amaleen, Baylie & Brittney - and -jr- Chaos for spooning with all -db- members.]]
